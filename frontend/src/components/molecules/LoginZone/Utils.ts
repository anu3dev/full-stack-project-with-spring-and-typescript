...
export const handleRegistrationBtnClick = async (
  ...
  if (formData.emailId === '') {
    ...
    formData.emailId !== '' &&
    (helperTextMessage === '' || helperTextMessage === 'Please enter your email ID.')
  ) {
    ...
    formData.otpValue === '' &&
    (helperTextMessage === `It's time to check your inbox for OTP.` || helperTextMessage === `Entered OTP is invalid.`)
  ) {
    ...
    (helperTextMessage === 'OTP verified successfully.' ||
      helperTextMessage === 'Please enter below missing details.') &&
    (formData.name === '' || formData.phoneNo === '' || formData.registeredBy === '' || formData.companyId === '')
  ) {
